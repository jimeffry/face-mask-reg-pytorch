...
class MobileNetV2(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = x.mean([2, 3])
        x = self.classifier(x)
        max_conf, _ = torch.max(x[:, 0:3], dim=1, keepdim=True)
        x = torch.cat((max_conf, x[3:,]), dim=1)
        x = self.softmax(x)
        return x
...
def mobilenet_v2(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        state_dict = model_zoo.load_url(model_urls['mobilenet_v2'], model_dir='.')
